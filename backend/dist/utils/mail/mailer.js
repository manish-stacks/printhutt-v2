...
exports.sendOtpBySms = exports.sendOtpByEmail = exports.sendVerifyEmail = void 0;
exports.sendOrderConfirmationEmail = sendOrderConfirmationEmail;
exports.sendOrderStatus = sendOrderStatus;
exports.sendRtoMessage = sendRtoMessage;
exports.sendDeliveredWithRatingMessage = sendDeliveredWithRatingMessage;
...
const owner_1 = require("./templates/owner");
const dotenv_1 = __importDefault(require("dotenv"));
const order_status_1 = require("./templates/order-status");
...
const sendVerifyEmail = async ({ email, emailType, userId }) => {
    try {
...
async function sendOrderConfirmationEmail(order) {
    ...
    const commonData = {
        ...
        coupon: order.coupon,
        formatCurrency: helpers_1.formatCurrency,
    ...
    try {
        await Promise.all([
            transporter.sendMail({
                ...
                html: (0, customer_1.getCustomerEmailTemplate)(commonData),
            }),
            transporter.sendMail({
                from: process.env.SMTP_FROM,
                to: process.env.SHOP_OWNER_EMAIL,
                subject: `New Order Received - ${order.orderId}`,
                html: (0, owner_1.getOwnerEmailTemplate)(commonData),
            }),
        ]);
    }
    catch (err) {
    ...
    💰 Total Amount: ${(0, helpers_1.formatCurrency)(order.totalAmount.discountPrice)}
    💰 Coupon Discount: ${order.coupon?.discountAmount ? (0, helpers_1.formatCurrency)(order.coupon?.discountAmount ?? 0) : 'N/A'}
    💵 Paid Amount: ${(0, helpers_1.formatCurrency)(order.payAmt)}
    ${order.paymentType === 'online' ? '' : `💵 COD Amount: ${(0, helpers_1.formatCurrency)(order.totalAmount.discountPrice - order.payAmt)}`}
    ...
    `;
        ...
        try {
            await axios_1.default.post(wappUrl);
        }
        catch (e) {
...
async function sendOrderStatus(order) {
    const emailContent = order.status === 'shipped'
        ? (0, order_status_1.getShippedEmailTemplate)(order)
        : (0, order_status_1.generateOrderStatusEmail)(order);
    ...
    if (order?.shipping?.mobileNumber) {
        ...
        const wappUrl = `${process.env.WAPP_URL}send?apikey=${process.env.WAPP_KEY}&mobile=${order.shipping.mobileNumber}&msg=${encodeURIComponent(wappMsg)}`;
        wappPromise = axios_1.default.post(wappUrl);
    }
    await Promise.all([emailPromise, wappPromise]);
}
async function sendRtoMessage(order, refundReason) {
...
async function sendDeliveredWithRatingMessage(order) {
    /* ================= EMAIL ================= */
