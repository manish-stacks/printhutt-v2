...
export const adminListQuerySchema = z.object({
  ...
  category: z.string().optional(),            // category _id
  status: z.enum(['all', 'active', 'inactive']).default('all'),
  stock: z.enum(['all', 'in', 'low', 'out']).default('all'),
  discount: z.enum(['all', 'yes', 'no']).default('all'),
  type: z.enum(['all', 'customize', 'variant', 'simple']).default('all'),
  minPrice: z.coerce.number().optional(),
  maxPrice: z.coerce.number().optional(),
  sort: z.enum(['newest', 'oldest', 'price_asc', 'price_desc', 'stock_asc', 'title']).default('newest'),
});
...
export const storefrontCategoryQuerySchema = z.object({
  ...
  seed: z.coerce.number().int().nonnegative().optional(), // random order (Flipkart style), stable per session
});
