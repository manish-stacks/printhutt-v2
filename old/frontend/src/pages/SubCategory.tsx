...
const Category = ({ subslug }: PropsInterface) => {
    ...
    const fetchProducts = useCallback(async (pageNum: number) => {
        try {
            ...
            const res: any = await productService.getProductsBySubCategory(pageNum, subslug, LIMIT);
            const newProducts = res?.products || [];

            setProductData((prev) => pageNum === 1 ? newProducts : [...prev, ...newProducts]);
            if (pageNum === 1) setCategoriesData(res?.categories || []);  // sirf first page pe
    ...
    return (
        <>
            <SubCategorySection slug={subslug} categories={categories} loading={loading} />
