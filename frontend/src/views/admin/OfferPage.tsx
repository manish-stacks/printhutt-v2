...
export default function OfferPage() {
    ...
    const lf = useListFilters(offers as any[], { nameKey: 'offerTitle', extra: [{ key: 'validity', label: 'Validity', options: [{ value: 'all', label: 'Any validity' }, { value: 'live', label: 'Live now' }, { value: 'expired', label: 'Expired' }, { value: 'upcoming', label: 'Upcoming' }], test: (r: any, v: string) => { const n = Date.now(), f = new Date(r.validFrom).getTime(), t = new Date(r.validTo).getTime(); return v === 'live' ? f <= n && n <= t : v === 'expired' ? t < n : f > n; } }] });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const success = editingId
                ? await modifyOffer(editingId, formData as any)
