...
import { effectivePricing, youtubeId, youtubeThumb, youtubeEmbed } from '@/lib/pricing';

/* gallery item: normal image url ya 'yt:<id>' (YouTube video) */
const ytOf = (u?: string) => (u && u.startsWith('yt:') ? u.slice(3) : null);
const toMedia = (url?: string) => {
  if (!url) return null;
  const id = youtubeId(url);
  return id ? `yt:${id}` : `${url}?auto=format&fit=crop&w=800&q=80`;
};
...
interface ProductProps {
  product: {
    ...
    weight?: number; isVarientStatus?: boolean; stock?: number; status?: boolean;
...
export default function ProductDetails({ product, relatedProduct }: ProductProps) {
  ...
  const storeCfg = useStoreSettings();

  useEffect(() => {
    if (product?._id) trackViewContent({ _id: product._id, title: product.title, price: product.price });
  }, [product?._id]);
  ...
  // ✅ Variant ka apna discount (admin set) — product discount alag
  const activePricing = effectivePricing(product as any, selectedVariant as any);
  const activeDiscountType = activePricing.discountType;
  const activeDiscountPrice = activePricing.discountPrice;
  ...
  const productImages = useMemo(() => {
    const demo = toMedia((product as any)?.demoVideo);
    const withVideo = (list: (string | null)[]) => {
      const out = list.filter(Boolean) as string[];
      if (demo && demo.startsWith('yt:') && !out.includes(demo)) out.splice(Math.min(1, out.length), 0, demo);
      return out;
    };
    // Variant ki apni images (isMainProduct ON ya images exist)
    ...
    if (variantHasImages || variantIsMain) {
      const list = [toMedia(selectedVariant?.thumbnail?.url), ...(selectedVariant?.images || []).map((img) => toMedia(img.url))];
      if (list.some(Boolean)) return withVideo(list);
    }
    // Fallback: product level (thumbnail YouTube link bhi ho sakta hai)
    return withVideo([toMedia(product?.thumbnail?.url), ...(product?.images || []).map((img) => toMedia(img.url))]);
  }, [selectedVariant, product]);
          ...
          <div className="fixed inset-0 bg-black bg-opacity-90 z-50 flex items-center justify-center">
            <div className="relative w-full max-w-6xl mx-4">
              ...
              <div className="relative flex items-center justify-center">
                {ytOf(productImages[currentImageIndex]) ? (
                  <iframe src={youtubeEmbed(ytOf(productImages[currentImageIndex])!)} title="video" className="w-full aspect-video" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen />
                ) : (
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="md:sticky md:top-8 space-y-4 h-fit">
            <div
              ...
              onMouseEnter={() => !isMobile && !ytOf(productImages[currentImageIndex]) && setIsZoomed(true)}
              onMouseLeave={() => !isMobile && setIsZoomed(false)}
            >
              {ytOf(productImages[currentImageIndex]) ? (
                <iframe
                  src={youtubeEmbed(ytOf(productImages[currentImageIndex])!)}
                    ...
                    >
                      {ytOf(image) ? (
                        <div className="relative w-full h-full">
                          <img src={youtubeThumb(ytOf(image)!)} alt="video" className="w-full h-full object-cover" />
          ...
          <div className="space-y-6 mt-8 md:mt-0">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`text-xs pb-1.5 ${isSelected ? 'text-pink-500 font-semibold' : 'text-gray-500'}`}>
                          ₹{effectivePricing(product as any, variant as any).final.toFixed(0)}
                        </div>
                        {isSelected && (
            ...
            {/* Countdown (admin: Settings → Storefront) */}
            {storeCfg.showDealCountdown && <div className="flex justify-between items-center bg-blue-50 p-2 sm:p-4 rounded-lg">
              <div className="font-semibold text-black text-xs sm:text-base">
                Sale ends in:{' '}
                <span className="font-bold">
                  <DealCountdown minutes={storeCfg.dealCountdownMinutes} />
                </span>
              </div>
          ...
          <div className="flex flex-wrap w-full">
            ...
            <div className="w-full px-[12px]">
              <div className="bb-deal-slider m-[-12px]">
                <ProductSlider products={relatedProduct} title="You may also like" description="Handpicked from the same collection — refreshed every visit." />
              </div>
            </div>
          </div>
        </div>
...
/* 1s timer apne component me — pehle poora ProductDetails page har second re-render hota tha */
function DealCountdown({ minutes = 59 }: { minutes?: number }) {
