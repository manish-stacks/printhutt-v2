...
export default function Page() {
  ...
  useEffect(() => {
    if (!sessionStorage.getItem("gift-refresh")) {
      sessionStorage.setItem("gift-refresh", "true");
  ...
  const handleAddToCart = async () => {
    const { discountPrice: cartTotal = 0 } = getTotalPrice();

    if (cartTotal < 1000) {
      toast.error("You must have orders worth ₹1000 or more to unlock this FREE gift.");
      return;
    }
    ...
    try {
      ...
      if (product) {
        ...
        const hasFree = items.some(i => i._id === FREE_ID);
        if (hasFree) removeFromCart(FREE_ID);
        ...
        openCartSidebarView();
        toast.success("🎁 Free Keychain added to your cart!");
      }
    } catch (error) {
              ...
              <div className="space-y-6">
                ...
                <div className='text-gray-600 text-md'>
                  <ul className='list-disc list-inside'>
                    <li>Upload your original high-quality image for the frame.</li>
                    <li>The recommended resolution is <span className='text-amber-500'>250 x 250</span> pixels at 300 DPI.</li>
                  </ul>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-yellow-400 text-slate-700 py-3 px-6 rounded-md font-medium hover:bg-yellow-500 flex items-center justify-center gap-2"
                    >
